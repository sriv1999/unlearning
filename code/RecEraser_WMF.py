...
class RecEraser_WMF(object):
    ...
    def _init_weights(self):
        ...
        all_weights['WA'] = tf.Variable(
            tf.random.truncated_normal(shape=[self.emb_dim, int(self.attention_size)], mean=0.0, stddev=tf.sqrt(
                tf.compat.v1.div(2.0, self.attention_size + self.emb_dim))), dtype=tf.float32, name='WA')
        ...
        all_weights['WB'] = tf.Variable(
            tf.random.truncated_normal(shape=[self.emb_dim, int(self.attention_size)], mean=0.0, stddev=tf.sqrt(
                tf.compat.v1.div(2.0, self.attention_size + self.emb_dim))), dtype=tf.float32, name='WB')
    ...
    def train_single_model(self, local_num):
        ...
        u_e_drop = tf.nn.dropout(u_e, rate=1 - (self.dropout_keep_prob_local))
    ...
    def attention_based_agg(self, embs, flag):
        if flag == 0:
            ...
            embs_w = tf.compat.v1.div(embs_w, tf.reduce_sum(embs_w, 1, keepdims=True))
        else:
            embs_w = tf.exp(
                tf.einsum('abc,ck->abk', tf.nn.relu(
